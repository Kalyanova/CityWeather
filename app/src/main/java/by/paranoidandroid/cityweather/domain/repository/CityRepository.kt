...
class CityRepository @Inject constructor(
        ...
        disposable = webService.getCityForecast(id)
            ...
            .subscribe(
                { forecast ->
                    ...
                    forecast.url = url
                    forecast.cityDescription = cityDescription
                    data.postValue(requireType(forecast))
                },
                { throwable ->
                    ...
                    dbRepository.getCityForecast(id)
                        ...
                        .subscribe(
                            { dbforecast ->
                                ...
                                data.postValue(requireType(dbforecast))
                            },
                            { dbthrowable ->
            ...
            disposable = webService.getForecasts(ids)
                ...
                .subscribe(
                    { forecastList ->
                        ...
                        val result = requireType<ForecastList>(forecastList.list.asList())

                        result.forEach {
                            val oldItem = oldData.firstOrNull { item -> item.id == it.id }
                            it.url = oldItem?.url
                            it.cityDescription = oldItem?.cityDescription
                        }
                        ...
                        val roomForecasts = Array(result.size, init = { index ->
                            ...
                            RoomForecast(
                                item.id, item.name, item.cityDescription,
                                coord, main, item.url
                            )
                        })
        ...
        disposable = dbRepository.getForecasts()
            ...
            .subscribe(
                { forecasts ->
                    ...
                    data.postValue(requireType(forecasts))
                },
                { throwable ->
    ...
    fun deleteForecast(cityName: String) {
        Log.d(LOG_TAG, "deleteForecast ${cityName}")
        dbDisposable = Completable
            .fromAction {
                dbRepository.delete(cityName)
            }
            .subscribeOn(Schedulers.io())
            .subscribeWith(object : DisposableCompletableObserver() {
                public override fun onStart() {
                    Log.d(LOG_TAG, "DB delete started")
                }

                override fun onError(ex: Throwable) {
                    Log.e(LOG_TAG, "DB delete error!", ex)
                }

                override fun onComplete() {
                    Log.d(LOG_TAG, "DB delete done!")
                }
            })
    }

    fun insertForecast(forecast: Forecast<Main, Coord>) {
        Log.d(LOG_TAG, "insertForecast ${forecast.name}")
        dbDisposable = Completable
            .fromAction {
                val coord = RoomCoord(forecast.coord?.lon, forecast.coord?.lat)
                val main = RoomMain(
                    forecast.main?.temp,
                    forecast.main?.minTemp, forecast.main?.maxTemp
                )
                val roomForecast = RoomForecast(
                    forecast.id,
                    forecast.name, forecast.cityDescription,
                    coord, main, forecast.url
                )

                dbRepository.insert(roomForecast)
            }
            .subscribeOn(Schedulers.io())
            .subscribeWith(object : DisposableCompletableObserver() {
                public override fun onStart() {
                    Log.d(LOG_TAG, "DB insert started")
                }

                override fun onError(ex: Throwable) {
                    Log.e(LOG_TAG, "DB insert error!", ex)
                }

                override fun onComplete() {
                    Log.d(LOG_TAG, "DB insert done!")
                }
            })
    }
